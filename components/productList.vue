    ...
    <transition name="fade" mode="out-in">
      <div v-if="showDeleteConfirmation" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
          <p class="text-center mb-4">{{ $t('alert.confirmDelete.product.message') }}</p>
          <div class="flex justify-center space-x-4">
            <button @click="deleteProduct" class="bg-red-600 text-white px-4 py-2 rounded-lg">{{ $t('button.yes') }}</button>
            <button @click="showDeleteConfirmation = false" class="bg-gray-400 text-white px-4 py-2 rounded-lg">{{ $t('button.no') }}</button>
          </div>
        </div>
      </div>
    </transition>
